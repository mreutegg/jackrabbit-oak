...
public class DocumentMK {
    ...
    private void parseAddNode(CommitBuilder commit, JsopReader t, String path) {
        ...
        addNodeToCommit(commit, n);
    }

    private void addNodeToCommit(CommitBuilder commit, DocumentNodeState node) {
        String path = node.getPath();
        UpdateOp op = node.asOperation(commit.getRevision());
        commit.addNode(path, op);
    }
    ...
    private void moveOrCopyNode(boolean move,
        ...
        addNodeToCommit(commit, newNode);
        if (move) {
